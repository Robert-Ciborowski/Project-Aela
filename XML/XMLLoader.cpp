...
bool Aela::XMLLoader::loadFromFile(std::string& src) {
	try {
		...
		AelaErrorHandling::consoleWindowError("XML Loader", "Could not open uncompressed file: " + src + "!");
		return false;
	}
...
bool Aela::XMLLoader::loadFromCompressedFile(std::string& src) {
	...
	if (!input.is_open()) {
		AelaErrorHandling::consoleWindowError("XML Loader", "Could not open compressed file: " + src + "!");
		input.close();
		return false;
	}
	...
	if (ret != CompressionError::OK) {
		AelaErrorHandling::consoleWindowError("World Loader", "An error occured while decompressing " + src + ": " + std::to_string(enumToInteger(ret)));
		input.close();
		return false;
	}
